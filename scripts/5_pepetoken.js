...
async function main() {
  const contractName = 'PEPE';
  const templateName = 'PepeToken';

  const _name = "Pepe Token";
  const _symbol = "$Pepe";
  const _totalsupply = BigInt(420690000000000) * BigInt(10 ** 18);
  const arguments = [_name, _symbol, _totalsupply];
